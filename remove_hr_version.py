...
def process_lua_file(filename):
    ...
    for i, line in enumerate(lines):
        ...
        if "hr_version" in stripped_line:
            ...
            if current_indent_level >= base_indentation + 2 and not skipped_block_now_delete:
                # Unindent by 2 spaces
                new_line = " " * (current_indent_level - 2) + stripped_line
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python remove_hr_version.py <folder_path>")
        sys.exit(1)
